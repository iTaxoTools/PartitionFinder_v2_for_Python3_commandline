...
class TextReporter(object):
    ...
    def write_best_scheme(self, result):
        ...
        if self.cfg.search in ["rcluster", "hcluster", "rclusterf"]:
            ...
            output.write(scheme_header_template % (b"weights", pretty_weights))

        if self.cfg.search.startswith("rcluster"):
            output.write(scheme_header_template % (b"rcluster-percent",
                                                   self.cfg.cluster_percent))
            output.write(scheme_header_template % (b"rcluster-max",
                                                       self.cfg.cluster_max))

        if self.cfg.search == "kmeans" or self.cfg.search == "krmeans":
            output.write(scheme_header_template % (b"min-subset-size",
                                                   self.cfg.min_subset_size))
            output.write(scheme_header_template % (b"kmeans based on",
                                                   self.cfg.kmeans))
            if self.cfg.all_states == True:
                output.write(scheme_header_template % (b"--all_states setting used",
                                                       self.cfg.all_states))
...
def write_raxml_partitions(sch, output, sorted_subsets, use_lg = False):
    ...
    for sub in sorted_subsets:
        ...
        output.write("%s, Subset%s = %s\n" % (model, subset_number, partition_sites))
        #output.write(b"\n")
        subset_number += 1
